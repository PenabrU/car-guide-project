...
def upload_file_and_analyze():
    ...
    if file:
        ...
        try:
            ...
            # [전략 변경!] Gemini 프롬프트 수정 (오류 수정 지시 추가)
            prompt = f"""
            당신은 중고차 성능기록부 분석 전문가이며, OCR 오류를 수정하는 능력도 뛰어납니다.
            ...
            [작업 지시]
            이 텍스트를 '당신이 직접 해석'하여, 아래 3가지 항목에 대해 분석 리포트를 생성해 주세요.
            
            [매우 중요!]
            OCR이 텍스트를 추출하는 과정에서 '차량 번호'나 '차대 번호'의 한글/숫자 조합을 혼동했을 수 있습니다.
            예를 들어, "6120836" 과 같이 숫자로만 조합된 이상한 차량 번호가 있다면, 이는 "61오0836"을 잘못 읽은 것일 확률이 매우 높습니다.
            '차명'이 '아반떼' 인 것을 고려할 때, 한국 차량 번호 체계(숫자+한글+숫자)에 맞게 문맥을 추론하여 '차량 번호'를 올바르게 수정해서 리포트에 포함시켜 주세요.

            [리포트 형식]
            각 항목은 Markdown을 사용하여 명확하게 구분하고, 친절한 말투를 사용해 주세요.

            1.  **차량 기본 정보:**
                (차명, 차량번호, 연식, 주행거리를 찾아서 요약. 차량번호는 당신이 '수정한' 번호로 기재)

            2.  **사고 및 수리 이력 분석 (가장 중요):**
                (텍스트에서 '사고이력', '교환', '판금' 등의 정보를 찾아 분석. 예: '프론트 휀더(우) 교환(X)' [cite: 27, 28] 이력이 있습니다. 이는 주행에...)

            3.  **잠재적 리스크 및 조언 (예상 비용 포함):**
                (텍스트에서 '누유', '미세누수' 등의 정보를 찾아 분석. 예: '엔진오일팬 미세누유' [cite: 36]가 체크되어 있습니다. 당장 수리는...)
            """
